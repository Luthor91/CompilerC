...
REM Lecture des paramètres du fichier settings.txt
if exist "settings.txt" (
    for /f "usebackq tokens=1,* delims=: " %%a in ("settings.txt") do (
        if "%%a"=="SOURCE_DIR" ( set "source_dir=%%b" 
        ) else if "%%a"=="PROJECT_DIR" ( set "project_dir=%%b" 
...
if not defined compile_dir ( 
    set "compile_dir=%project_dir%\build" 
    echo compile dir not defined
)
if defined LIB_DIR (
    ...
    for %%D in (!external_directories:,= !) do (
        ...
        for %%C in ("%current_directory%\*.c") do (
            ...
            for %%i in (%ignore_list%) do (
                if "%%~dpnxF" equ "%current_directory%\%%~i" (
                    set "skip_file=1"
                )
            )
            if not defined skip_file (
                ...
                set "relative_path=!relative_path:%current_directory%=!"
                set "relative_path=!relative_path:\=!"
                set "build_path=%compile_dir%!relative_path!"
        ...
        for %%H in ("%current_directory%\*.h") do (
            ...
            set "skip_file="
            for %%i in (%ignore_list%) do (
                set "compare_path=%current_directory%\%%~i"
                if "%%~dpH%%~nxH" equ "%current_directory%\%%~i" (
                    set "skip_file=1"
                )
            )
            if not defined skip_file (
        ...
        for %%F in ("%current_directory%\*.a" "%current_directory%\*.dll") do (
            set "compilation_success=1"
            echo Fichier bibliothèque trouvé : "%%F"
            for %%i in (%current_directory%) do (
                set "filename=%%~nxf"
...
:CREATE_DIRECTORY
    set "log_file=%project_dir%\logs\%date_stamp%_%time_stamp%.txt"
    if not exist "%project_dir%\logs" mkdir "%project_dir%\logs"
    if not exist "%project_dir%\build" mkdir "%project_dir%\build"
    if not exist "%project_dir%\exe" mkdir "%project_dir%\exe"
exit /b
...
:COMPILE_SOURCE_FILES
    for /r "%source_dir%" %%F in (*.c) do (
        ...
        if not defined skip_file (
            ...
            set "relative_path=!relative_path:\=!"
            REM Ne pas créer le répertoire "build" ici
            set "build_path=%compile_dir%!relative_path!"
            REM Compilation avec redirigeant les erreurs vers le fichier log
            echo Compilation de "%%~nF.c" : >> "%log_file%"
            gcc -c "%%F" -o "!build_path!\%%~nF.o" -Wall >> "%log_file%" 2>&1
